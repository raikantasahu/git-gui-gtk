...
def _create_single_line_hunk(hunk_lines: list[str], line_in_hunk: int) -> list[str]:
    """Create a hunk that only stages a single line.

    Args:
        hunk_lines: The original hunk lines (including @@ header)
        line_in_hunk: Index of the line to stage within the hunk (0 = @@ header)
    ...
    if not target_line or target_line[0] not in ['+', '-']:
        return []  # Can't stage a context line
    ...
    for i, line in enumerate(hunk_lines[1:], start=1):
        ...
        if i == line_in_hunk:
            ...
            # But we skip them for staging - they stay as working tree changes
            # Actually, we need to not include them at all in the patch
            pass
        elif line_type == '-':
...
def stage_line(
    ...
    """Stage a specific line from a file.
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return True, f'Staged {line_type} line from {file_path}'
        else:
            error = result.stderr.strip() or result.stdout.strip()
            return False, f'Failed to stage line: {error}'

    except Exception as e:
        return False, f'Error staging line: {e}'
