...
def stage_hunk(
    ...
    try:
        ...
        if hunk_idx == -1:
            # If cursor is in header, stage first hunk
        ...
        if result.returncode == 0:
            return True, f'Staged hunk {hunk_idx + 1} of {len(hunks)} from {file_path}'
        else:
            error = result.stderr.strip() or result.stdout.strip()
            return False, f'Failed to stage hunk: {error}'

    except Exception as e:
        return False, f'Error staging hunk: {e}'
